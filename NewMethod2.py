...
def transform_df(df):
    ...
    for c in dcol:
        if '_bin' not in c: #standard arithmetic
            ...
            df[c+str('_sq')] = df['ps_car_13'] * np.sqrt(df[c].values).astype(np.float32) * np.sign(df[c].values)
...
params = {'eta': 0.07, 'max_depth': 4, 'objective': 'binary:logistic', 'subsample': 0.8, 'colsample_bytree': 0.8, 'min_child_weight': 0.77, 'scale_pos_weight': 1.6, 'gamma': 10, 'reg_alpha': 8, 'reg_lambda': 1.3, 'eval_metric': 'auc', 'seed': 99, 'silent': True}
